...
[![LOC](https://img.shields.io/badge/LOC-175789-blue)](#-项目统计)
...
| **📏 基线模式** | 历史样本不足时显示“基线建立中 / 即时洞察”，避免伪造稳定趋势或统计结论 |
...
| **⚡ 阶段进度跟踪** | 0-90% 为阶段性进度估算，`SUCCESS / FAILED` 为可靠终态 |
| **🛡️ 超时与失败收口** | 默认 180 秒超时保护，预处理/模型异常会统一落为失败态 |
| **📈 结果展示** | 详细的诊断结果和置信度             |
| **💡 临床建议** | 提供专业的临床建议和生物标志物信息 |
| **🧭 结果标准化** | AI 输出、手动结果写回、患者洞察与报告导出统一走结果标准化内核，风险等级与建议口径一致 |
| **📄 报告生成** | 自动生成医生归档版 PDF 报告，支持影像对比、关键指标、可疑区域、历史趋势与免责声明 |
| **🎛️ 增强视图控制** | 支持放大、缩小、全屏控制，AI 标注视图实时交互 |
| **🛑 分析取消** | 支持分析过程中取消操作            |
| **⚠️ 错误重试** | AI 分析失败时显示错误信息并提供重新分析入口 |
| **🎚️ 参数配置** | 支持模型选择、灵敏度与置信度阈值调节 |
| **🧾 区域摘要卡** | 病例详情页改为全宽区域摘要卡展示离散病灶区域，避免与上方原图/标注图重复 |
...
| **🧩 通知模块拆分**         | 主布局回归装配层，通知铃铛、导航抽屉、用户菜单与通知逻辑已拆为独立模块 |
...
| **📨 手动提醒**             | 支持单条随访“立即提醒”，并向计划接收人及当前操作者同步投递站内通知  |
...
| **🧪 前端静态校验补齐**     | 新增 `npm run typecheck` 与回归检查清单，便于大改后快速做静态收口    |
| **📊 系统监控面板**        | 实时 CPU 负载、内存使用、数据库连接池与分析队列状态监控              |
| **📥 患者批量导入**         | 支持 `.xlsx` 文件拖拽上传、预览确认与模板下载                         |
...
| **📜 订单管理** | 订单历史查询和状态跟踪             |
| **🔄 积分系统** | 购买套餐自动发放积分               |
| **🧭 页面拆分** | 订阅页聚焦套餐权益，AI 引擎与服务偏好独立到专门设置页 |
| **🧩 SaaS 定价重构** | 订阅页收敛为标准 SaaS pricing page，当前订阅状态、套餐主卡、能力矩阵与信任区分层更清晰 |
| **🎚️ 动态档位展示** | 顶部筛选联动周期套餐 / 单次开通；按月场景支持“连续包月 / 一月版”双档位切换 |
| **📈 临床指标可视化** | 模型指标区升级为“左侧雷达图 + 右侧核心数字”，强化 AI 医疗平台的专业展示力 |
| **✨ 微交互动效** | 套餐卡、切换器、软著卡与核心数字补齐显隐过渡、悬停浮起、按压反馈与数字滚动动画 |
...
| **📑 历史报告** | 访问历史分析报告                |
| **🧾 医生归档版 PDF** | 输出首页摘要、原始影像/AI 标注对比、关键指标、可疑区域明细、患者趋势曲线、临床建议与免责声明 |
| **🧭 历史趋势整合** | 导出时拉取患者最近 6 次分析结果，用于展示风险权重与置信度变化 |
| **🖼️ 远程影像兼容** | 前端即时导出与后端报告生成均兼容图仓 URL，失败时提供占位说明与本地回退 |
| **⬇️ 报告下载** | 支持 PDF 格式下载               |
...
- **2026-03-21**：前端 UI 细节打磨：覆盖 15+ 页面无障碍 ARIA 标签、键盘 `:focus-visible` 样式、Dashboard 数字滚动动画（`useCountUp`）、欢迎语分时段问候、ReportsPage/PatientsPage 暗色模式完善、上传进度定时器内存泄漏修复、控制台日志清理。
- **2026-03-20**：分析任务队列化重构：`simpleAnalysisQueue.service.js` 实现内存队列（最大并发 3），通义千问超时从 60s 延长至 180s 并优化重试延迟与日志。
...
> 前端病例中心已改为“查询状态驱动”模式，`status / patient_id / keyword / page / rowsPerPage` 会与 URL 查询参数双向同步。
...
| :--- | :-------------- | :------------ |
| GET  | `/`             | 获取报告列表  |
| POST | `/generate`     | 生成 PDF / Word / Excel 报告 |
| POST | `/batch-export` | 批量导出报告 ZIP |
| GET  | `/:id`          | 获取报告详情  |
...
| POST | `/`                     | 创建随访计划                   |
| PUT  | `/:id`                  | 编辑随访计划                   |
| PATCH | `/:id/complete`        | 标记随访完成                   |
| PATCH | `/:id/cancel`          | 取消随访计划                   |
| PATCH | `/:id/high-attention`  | 切换医生重点关注状态           |
| POST | `/:id/remind`           | 立即发送站内提醒               |

> `POST /api/followups/:id/remind` 会去重后通知 `assigned_doctor_id`、`created_by` 与当前操作者；响应体保留 `notification`，并新增 `notifications` 数组承载本次实际创建/命中的通知集合。
...
| 病例影像存储 | 先本地持久化至 `server/uploads/`，再异步同步图仓；分析前优先使用图仓远程 URL，失败时自动回退本地路径 |
| 头像存储     | 内存缓冲区直传图仓，远程 URL 写入 `avatar_url` 字段 |
...
> 当前项目已完成"AI 辅助诊断"的核心技术闭环，但同类产品在正规医院已普及。以下四个方向旨在跳出"拍片 → AI 看片 → 出报告"的同质化框架，探索真正的差异化创新路径。

### 方向一：从"检测"转向"预测" — 宫颈癌风险预测引擎

**核心理念**：不做"看到病变告诉你有病变"的事后诊断，做"还没看到病变就告诉你未来风险多高"的事前预测。

| 维度     | 当前模式                     | 创新方向                                   |
| :------- | :--------------------------- | :----------------------------------------- |
| **输入** | 一张宫颈影像                 | HPV 分型 + TCT + 年龄 + 免疫状态 + 生活习惯 + 既往筛查史 |
| **输出** | "有没有病变"                | "未来 1/3/5 年进展为 CIN2+ 的概率"         |
| **模型** | 单帧图像分类                 | 多维度时序风险动态模型                     |
| **价值** | 单次诊断辅助                 | 个性化筛查间隔决策支持                     |

- 参考 ASCCP 2019 Risk-Based Management 指南，将临床决策规则编码为 AI 可执行的个性化动态模型
- 每次新数据输入，风险曲线实时更新，实现"越查越精准"
- **创新本质**：范式转换 — 从"不拍片不能判断"变成"不拍片也能算风险"

---

### 方向二：从"医生工具"转向"患者自主" — 居家自采样筛查平台

**核心理念**：中国宫颈癌筛查率低的根因不是技术不够，是女性不愿意去查。羞耻感、时间成本、交通成本才是真正的壁垒。

| 维度       | 当前模式             | 创新方向                               |
| :--------- | :------------------- | :------------------------------------- |
| **场景**   | 医院/诊所            | 居家自助完成                           |
| **驱动方** | 医生主导             | 患者主导，医生兜底                     |
| **采集**   | 医生操作阴道镜       | HPV 自采样试剂盒（已有成熟产品）       |
| **流程**   | 挂号 → 排队 → 检查   | App 风险自评 → 试剂盒寄送 → 结果回传   |

- 用户在 App 完成风险问卷（HPV 暴露史、疫苗接种、性伴侣数量等）
- 系统生成个性化筛查计划（"建议每年查一次" 或 "可 3 年后再查"）
- 对接 HPV 自采样试剂盒：在家采样寄送，结果回传 App
- AI 结合自采样结果 + 问卷数据，给出综合风险评估
- **创新本质**：把宫颈癌筛查从"去医院"变成"在家完成"，从根本上解决筛查率低的社会痛点

---

### 方向三：从"单次检测"转向"终身轨迹" — 宫颈健康数据主权平台

**核心理念**：现在没有任何产品给女性一个"属于自己的宫颈健康档案"。数据都在医院系统里，患者看不到、带不走、换医院就断了。

| 维度       | 当前模式                   | 创新方向                             |
| :--------- | :------------------------- | :----------------------------------- |
| **数据**   | 每次检测孤立，报告看完就完 | 终身宫颈健康数字档案，串联所有记录   |
| **归属**   | 数据存在医院 HIS 系统      | 患者拥有数据主权，可携带、可授权     |
| **分析**   | 单次结果判断               | 时间维度风险演变图谱，AI 标注转折点  |
| **互操作** | 各医院数据孤岛             | FHIR 标准导出，跨院转诊无需重复检查  |

- 从 25 岁首次筛查开始，建立个人宫颈健康时间线
- 每次 HPV 检测、TCT、活检、疫苗接种、治疗记录全部串联
- AI 自动标注关键转折点（"2025 年 HPV16 转阳" → "2026 年 LSIL 出现" → "2027 年转阴"）
- 生成个人宫颈健康报告卡，可授权分享给不同医生
- **创新本质**：从"AI 检测工具"变成"宫颈健康数据主权平台"

---

### 方向四：从"辅助诊断"转向"消灭宫颈癌" — 区域消除作战平台

**核心理念**：从"帮一个医生看好一个病人"变成"帮一个地区消灭一种癌症"。呼应 WHO 2030 消除宫颈癌战略。

| 维度         | 当前模式                 | 创新方向                             |
| :----------- | :----------------------- | :----------------------------------- |
| **服务对象** | 单个医生/单家医院        | 区域卫健委/疾控中心                  |
| **目标**     | 提高诊断准确率           | 提高区域筛查覆盖率，量化消除进度     |
| **数据**     | 单病例维度               | 区域维度公共卫生决策看板             |
| **闭环**     | 筛查 → 出报告            | 筛查 → 转诊 → 确诊 → 治疗 → 随访全链路追踪 |

- 为区域搭建宫颈癌消除作战平台：地图展示适龄女性总数、已筛查人数、覆盖率、阳性率、转诊完成率
- AI 自动识别高危未筛查人群，主动推送筛查提醒
- 追踪每位异常结果患者的转诊-确诊-治疗全链路，确保"筛了就要管到底"
- 产出可量化指标："使用系统后，本地区宫颈癌筛查率从 X% 提升到 Y%"
- **创新本质**：产品价值从"医疗效率工具"升级为"公共卫生基础设施"

---

### 🎯 推荐组合策略

> **方向二 + 方向三**：患者自主筛查 + 终身健康轨迹

| 组合优势   | 说明                                                                 |
| :--------- | :------------------------------------------------------------------- |
| **痛点最尖锐** | 中国筛查率低的根因不是技术，是"女性不去查"                       |
| **技术可落地** | 已有前端、后端、AI、用户系统，加自采样对接 + 健康档案无需重写架构 |
| **故事最打动人** | "让每一位女性在家就能掌控自己的宫颈健康"，比"AI 辅助诊断"有温度 |
| **政策契合** | 呼应 WHO 2030 消除宫颈癌战略、国家两癌筛查政策                      |
