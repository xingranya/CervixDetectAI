...
3. [布局组件设计](#布局组件设计)
4. [页面组件分析](#页面组件分析)
5. [AIChatPanel 组件（病例详情页）](#aichatpanel-组件病例详情页)
6. [基础UI组件封装](#基础ui组件封装)
7. [组件通信机制](#组件通信机制)
8. [响应式设计实现](#响应式设计实现)
9. [组件嵌套关系图示](#组件嵌套关系图示)
...
### AIChatPanel 组件（病例详情页）

`StudyDetailPage.vue` 已集成 `AIChatPanel.vue`，用于围绕当前病例分析结果进行追问和解释。该组件与普通表单页面不同，属于典型的长连接交互组件，核心特性如下：

- **双模式交互**：支持“深度思考模式 / 快速回复模式”切换。
- **流式展示**：区分 `reasoning`（思考过程）与 `content`（正式回答）并分别渲染。
- **中断控制**：通过 `AbortController` 中止正在进行的流式回复。
- **滚动策略**：消息区与思考区分别自动滚动到最新位置。
- **输出渲染**：AI 正文支持 Markdown 渲染并通过 DOMPurify 做安全净化。

### StudyDetailPage 病灶摘要区（近期更新）

`StudyDetailPage.vue` 中的“病变特征识别结果”区域已从早期的柱形图方案收敛为 **全宽区域摘要卡**，用于表达离散病灶区域的快速摘要，而不是统计图表。

该区域的设计原则：

- **不重复上方影像对比**：页面顶部已经有“原图 + AI 标注图”完整对比，因此病灶摘要区不再重复展示缩略图或第二套影像导航
- **只负责快速读结论**：聚焦区域编号、位置、复核优先级、简要描述和关键特征
- **向后兼容**：同时兼容
  - 历史病例仅返回字符串描述的区域数据
  - 当前病例返回结构化区域对象（`description / location / features / box_2d / bbox_2d`）
  - 无独立区域时的统一空态说明
- **自适应网格布局**：
  - 1 个区域：单卡全宽
  - 2 个区域：双卡片并排
  - 3 个及以上区域：自动切换为自适应网格，窄屏回落为单列

与该区域配套的前端状态派生包括：

- `lesionSummaryCards`
- `lesionSummaryGridClass`
- `lesionMetricSummary`
- `suspiciousAreaSummaryLabel`

这一调整使病例详情页的“病变特征”表达从“像报表”转向“像病例摘要”，更符合医生快速判读场景。

```mermaid
sequenceDiagram
participant SD as StudyDetailPage
participant Panel as AIChatPanel
participant Service as chatService
participant API as /api/chat(SSE)
SD->>Panel : 打开聊天面板并传入 studyId
Panel->>Service : sendChatMessage(studyId, message, history, enableThinking)
Service->>API : POST /api/chat
API-->>Service : reasoning/content 流式分片
Service-->>Panel : onChunk(type, text)
Panel->>Panel : 更新思考卡片与正文气泡
Panel->>Service : stopResponse() 时触发 abort
```
