...
- `POST /api/studies/:id/images`
  - 主事务先完成本地持久化与数据库记录写入
  - 提交后会立即尝试调用 `syncStudyImageToTucang(...)`
  - 响应返回前会通过 `serializeStudyImageForResponse(...)` 优先输出图床直链
  - 图仓同步失败时仅记录警告，保留本地路径作为回退
...
- `resolvePreferredStudyImagePath(...)` 会优先采用标准图床 URL
- 若 `stored_filename` 已变为图仓 `md5`，会重建稳定的图床直链
- 历史错误主机名（如 `https://uploads/...`）会在响应阶段被纠正，避免前端继续请求错误域名
...
- `prepareStudyImageForAnalysis(...)` 的执行顺序：
  1. 若 `file_path` 或 `stored_filename` 已可推导出远程 URL，直接使用
  2. 否则先尝试同步图仓
  3. 若同步失败，再回退本地绝对路径
...
- `POST /api/analyze`
  - 事务提交后会先尝试同步病例影像到图仓
  - 再向前端返回任务创建成功响应
  - 这样可减少详情页在“分析中”阶段先读到本地 `/uploads/...` 路径的时间窗
...
- ✅ 对外影像路径已增加标准化与历史异常值纠正，减少前端拿到错误 `uploads` 域名的概率
