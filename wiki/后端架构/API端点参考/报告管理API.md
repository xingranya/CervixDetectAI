...
## 简介
报告管理API是CervixDetectAI系统的核心功能之一，用于管理和操作医疗分析报告。该API支持创建、生成、获取、更新、下载和删除报告等操作，所有端点均需通过JWT认证。报告与病例、患者和AI分析结果紧密关联，确保数据的完整性和可追溯性。
...
报告内容以JSON格式存储在数据库中，确保数据的可读性和可扩展性。
