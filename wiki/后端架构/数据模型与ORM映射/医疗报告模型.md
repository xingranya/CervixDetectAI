...
| storage_provider | ENUM('local', 'edgeone-blob') | 否 | 默认值'local' | 报告文件存储提供方 |
| storage_key | STRING(500) | 是 | 无 | 对象存储内部对象键 |
| storage_namespace | STRING(255) | 是 | 无 | 对象存储命名空间或桶标识 |
| storage_url | STRING(1000) | 是 | 无 | 存储服务返回的对象地址（当前可为空） |
| storage_status | ENUM('pending', 'completed', 'failed') | 否 | 默认值'completed' | 报告文件存储状态 |
...
STRING(500) storage_key
STRING(255) storage_namespace
STRING(1000) storage_url
ENUM storage_status
BIGINT file_size
INTEGER page_count
...
+复合索引: storage_provider + storage_status
