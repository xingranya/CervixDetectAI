...
- **sexual_history**：ENUM 枚举类型，取值为 'none'、'regular'、'irregular'、'multiple_partners'、'early_sexual_activity'、'other'，允许为空，默认值 'none'，注释为"性生活史"。
- **id_card**：STRING(50) 类型，允许为空，注释为"身份证号（加密存储）"，用于安全地保存身份信息。
- **medical_card_no**：STRING(50) 类型，允许为空，注释为"医保卡号"，用于存储医疗保险卡号码。
...
- **emergency_relation**：STRING(50) 类型，允许为空，注释为"紧急联系人关系"，用于记录紧急联系人与患者的关系。
- **allergy_history**：TEXT 类型，允许为空，注释为"过敏史"，用于记录过敏情况。
- **medical_history**：TEXT 类型，允许为空，用于记录患者的病史信息。
- **family_history**：TEXT 类型，允许为空，注释为"家族病史"，用于记录家族遗传病史。
- **notes**：TEXT 类型，允许为空，注释为"备注"，用于存储其他附加信息。
- **created_by**：BIGINT 类型，允许为空，外键引用 users 表的 id 字段，更新时级联（CASCADE），删除时设置为 NULL（SET NULL）。
