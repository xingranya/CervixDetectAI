...
3. [阿里云 ESA AI 验证码集成](#阿里云-esa-ai-验证码集成)
...
项目引入了阿里云 ESA (Edge Security Acceleration) AI 验证码服务，用于在登录、注册等环节进行人机验证，防止自动化脚本攻击。

详细集成文档请参阅：[阿里云AI验证码集成](阿里云AI验证码集成.md)

### 核心配置

- **AppKey**: `xxx`
- **Scene**: `xxx` (默认场景)
- **组件**: `src/components/common/AliCaptcha.vue`
...
| 环境变量                     | 用途                    | 示例值                                 |
| :--------------------------- | :---------------------- | :------------------------------------- |
| `QWEN_API_KEY`             | 通义千问API的认证密钥   | `sk-fxxx`                            |
