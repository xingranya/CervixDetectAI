...
```mermaid
flowchart TB
UI[Login/Register Page] --> API[src/services/api.ts]
API -->|POST /send-code| Router[sms-auth.js]
Router -->|检查频率/限制| DB[(MySQL)]
Router -->|生成验证码| Service[sms.service.js]
Service -->|发送短信请求| AliSMS[阿里云短信服务]
