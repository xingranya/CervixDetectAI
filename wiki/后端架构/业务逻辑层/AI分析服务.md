...
- 新增 `analysisResultNormalizer.service.js`，统一处理诊断结论、置信度、风险等级、可疑区域、生物标志物、临床建议与详细报告的标准化
- 自动分析、手动保存分析结果、患者洞察与报告导出改为共享同一套结果归一化逻辑，避免模块间口径漂移
...
### 2026-05-18 结果标准化与风险口径统一

- **新增结果标准化内核** `analysisResultNormalizer.service.js`
  - 统一归一化 `diagnosis`
  - 统一归一化 `confidence`
  - 统一归一化 `risk_level / riskLevel`
  - 标准化 `recommendations / biomarkers / suspiciousAreas / detailedReport`
- **保留持久化风险等级优先级**
  - 当结果已存在 `risk_level` 时，优先保留该等级
  - 仅在缺失时才回退到诊断文本或模型风险评估
- **无法诊断原因收口**
  - 不再粗暴覆盖已有原因
  - 缺失原因时才补“图像质量不足”或“原因待复核”
- **无局灶病灶时不再伪造区域**
  - `suspiciousAreas` 为空时保持真实计数为 `0`
  - 由前端和报告展示层承担自然语言说明兜底
