...
### 存储方案
- **HttpOnly Cookie（已实施）**：refreshToken 已通过 `HttpOnly Cookie` 存储，JavaScript 无法直接访问，有效防止 XSS 攻击窃取 Token。生产环境同时启用 `Secure` 标志，确保仅在 HTTPS 下传输。
- **SameSite属性**：Cookie 设置 `SameSite=Lax`，在大部分导航场景下提供 CSRF 保护。
